...
if(file_exists("./images/mini/" . $_GET["pict"]))
{
   imagejpeg(imagecreatefromjpeg("./images/mini/" . $_GET["pict"]));
}
else
{
   miniature($_GET["pict"], $_GET["size"]);
}
function miniature($pict, $size, $dest_dir="./images/mini/")
{  
   $handle = imagecreatefromjpeg("./images/big/" . $pict);
   //imagejpeg($handle);
   //exit();
   $x=imagesx($handle);
   $y=imagesy($handle);
   ...
   if(!imagejpeg($black_picture,$dest_dir. $size_in_pixel . "px_" . $pict, $size_in_pixel))
