...
$db1 = DBI->connect("DBI:mysql:aubri_cookbook;host=localhost","aubri", "z00l00");

$db2 = DBI->connect("DBI:mysql:oohaheats;host=localhost",, "aubri", "z00l00");
...
$db2->prepare("truncate categories;")->execute();
$db2->prepare("truncate groceries;")->execute();
...
while(@recrow = $rech->fetchrow())
{
   ...
   $recsel->execute();
   @recrow2 = $recsel->fetchrow();
   if(!defined(@recrow2[0]))
   {
      ...
      while(@row = $ingstmt->fetchrow())
      {
         ...
         $ingstmt2 = $db2->prepare("insert into ingredients (recipe_id, order_num, measurement_id, grocery_id, created_at, updated_at) values (?, ?, ?, ?, ?, ?);");
         $ingstmt2->bind_param(1, $new_recipe_id);
         $ingstmt2->bind_param(2, $ordernum);
         $ingstmt2->bind_param(3, $new_msrmnt_id);
         $ingstmt2->bind_param(4, $new_grocery_id);
         $ingstmt2->bind_param(5, '2010-02-06 03:26:36');
         $ingstmt2->bind_param(6, '2010-02-06 03:26:36');
         $ingstmt2->execute() or die "Error on insert: " . $DBI::errstr . "\n"
         ...
         println("============================");
         $ingstmt2->finish();
      } # END while(@row = $ingstmt->fetchrow())
   } # END if(!defined(@recrow2[0]))
   $recsel->finish();
} # END while(@recrow = $rech->fetchrow())
$rech->finish();
$db1->disconnect();
$db2->disconnect();
