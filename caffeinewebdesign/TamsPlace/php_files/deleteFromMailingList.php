...
?>
<H2 ALIGN = CENTER> The following entries have been removed from the mailing list: </H2>
...
<?php
	...
	while($row = mysql_fetch_array($result)){
		if (isset($$row["id"])){
			 ...
			 */
		    $sql = "DELETE FROM mailingList WHERE counter = " . $row["counter"];
			...
			$renumberSQL = "UPDATE mailingList SET ID = ID - 1 WHERE counter >= " . $row["counter"];
...
<H2 ALIGN = CENTER> Here is your new full mailing list: </H2>
...
<?php
    ...
    // Request the text of all the info
   	$result = mysql_query("SELECT * FROM mailingList ORDER BY ID");
  	if (!$result) {
    	echo("<P>Error performing query: " .
        mysql_error() . "</P>");
    	exit();
